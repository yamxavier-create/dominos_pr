 ...
 */
function syncPlayerSocketIds(game, rooms) {
...
function broadcastState(io, game, rooms) {
    if (rooms)
...
function handleBlockedGame(io, game) {
    ...
    if (isModo200) {
        ...
        const blockingTeam = (0, GameEngine_1.playerTeam)(lastTile.playedByIndex);
        blockBonusPoints = 2;
        const blockResult = (0, GameEngine_1.applyScore)(game.scores, blockingTeam, 2, game.targetScore);
    ...
    let pointsScored;
    if (isModo200) {
        const result = (0, GameEngine_1.calculateBlockedResult200)(game.players);
    ...
    if (winningTeam !== null) {
        const scoreResult = (0, GameEngine_1.applyScore)(game.scores, winningTeam, pointsScored, game.targetScore);
        updatedScores = scoreResult.updatedScores;
        if (scoreResult.gameOver)
            gameOver = true;
        game.scores = updatedScores;
    }
 ...
 */
function checkRematchCancellation(io, room, disconnectedSocketId) {
    if (!room.game || !room.rematchVotes || room.rematchVotes.length === 0)
        return;
...
function registerGameHandlers(socket, io, rooms) {
    ...
    socket.on('game:play_tile', ({ roomCode, tileId, targetEnd }) => {
        ...
        if (player.tiles.length === 0) {
            ...
            const { updatedScores, gameOver } = (0, GameEngine_1.applyScore)(game.scores, winningTeam, totalPoints, game.targetScore);
            game.scores = updatedScores;
            game.phase = gameOver ? 'game_end' : 'round_end';
            // Winner of this hand starts the next hand/game
            game.handStarterIndex = player.index;
            if (gameOver)
                game.gameWinnerIndex = player.index;
    ...
    socket.on('game:rematch_vote', ({ roomCode }) => {
        ...
        if (!room.rematchVotes)
            room.rematchVotes = [];
        if (room.rematchVotes.includes(player.index))
            return;
