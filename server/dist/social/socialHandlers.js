...
function registerSocialHandlers(socket, io, rooms, presence) {
    ...
    socket.on('social:friend_request', async ({ targetUserId }) => {
        try {
            ...
            const pendingCount = await prisma_1.default.friendship.count({
    ...
    socket.on('social:friend_accept', async ({ requestId }) => {
        try {
            const userData = (0, authMiddleware_1.getSocketUser)(socket);
            if (!userData.user) {
                return socket.emit('social:error', { message: 'Login required' });
            ...
            const userId = userData.user.id;
            const friendship = await prisma_1.default.friendship.findUnique({ where: { id: requestId } });
            if (!friendship) {
    ...
    socket.on('social:friend_reject', async ({ requestId }) => {
        try {
            const userData = (0, authMiddleware_1.getSocketUser)(socket);
            if (!userData.user) {
                return socket.emit('social:error', { message: 'Login required' });
            ...
            await prisma_1.default.friendship.delete({ where: { id: requestId } });
    ...
    socket.on('social:friend_remove', async ({ friendUserId }) => {
        try {
            const userData = (0, authMiddleware_1.getSocketUser)(socket);
            if (!userData.user) {
                return socket.emit('social:error', { message: 'Login required' });
            ...
            const result = await prisma_1.default.friendship.deleteMany({
                where: {
                    status: 'ACCEPTED',
                    OR: [
    ...
    // --- social:check_users ---
    socket.on('social:check_users', async ({ userIds }, callback) => {
        try {
            const userData = (0, authMiddleware_1.getSocketUser)(socket);
            if (!userData.user) {
            ...
            for (const uid of filteredIds) {
                const f = friendships.find(fr => fr.requesterId === uid || fr.targetId === uid);
                if (f) {
                    statusMap[uid] = {
                        status: f.status,
    ...
    // --- social:join_friend ---
    socket.on('social:join_friend', async ({ friendUserId }) => {
        try {
            const userData = (0, authMiddleware_1.getSocketUser)(socket);
